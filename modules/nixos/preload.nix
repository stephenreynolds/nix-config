...
let
  ...
  configFile = iniFormat.generate "preload.conf" cfg.settings;
in
{
  options.services.preload = {
    ...
    package = mkOption {
      ...
      defaultText = literalExpression "pkgs.preload";
      description = mdDoc ''
  ...
  config = mkIf cfg.enable {
    ...
    environment.etc."preload.conf".source = configFile;
    ...
    systemd.services.preload = {
      ...
      serviceConfig = {
        ...
        ExecStart = "${cfg.package}/bin/preload --foreground $PRELOAD_OPTS";
      };
    };
  };
}

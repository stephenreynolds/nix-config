...
in
{
  ...
  // lib.mkIf config.modules.system.security.firejail.enable {
    programs.firejail.wrappedBinaries = {
      gpg = {
        executable = "${pkgs.gnupg}/bin/gpg";
        profile = "${pkgs.firejail}/etc/firejail/gpg.profile";
      };
      gpg-agent = {
        executable = "${pkgs.gnupg}/bin/gpg-agent";
        profile = "${pkgs.firejail}/etc/firejail/gpg-agent.profile";
      };
    };
  };
}

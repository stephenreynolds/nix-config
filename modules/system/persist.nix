...
in {
  ...
  config = mkIf cfg.enable {
    environment.persistence = {
      "${cfg.state.path}" = {
        hideMounts = true;
        directories = [ "/var/lib/systemd" "/var/lib/nixos" "/var/log" ]
          ++ cfg.state.directories;
        files = [ "/etc/machine-id" ] ++ cfg.state.files;
      ...
      "${cfg.cache.path}" = {
        hideMounts = true;
        directories = [ "/var/cache" ] ++ cfg.cache.directories;
        files = cfg.cache.directories;
        users.${config.user.name} = {
          directories = [ ".cache" ".config/dconf" ]

...
let
  cfg = config.my.desktop.tiling-wm.wayland.gtklock;
...
in
{
  options.my.desktop.tiling-wm.wayland.gtklock = {
    ...
    config = {
      gtk-theme = lib.mkOption {
        type = lib.types.str;
        default = config.my.desktop.theme.gtk.theme.name;
  ...
  config = lib.mkIf cfg.enable {
    home.packages = [ cfg.package cfg.lockCommand ];
